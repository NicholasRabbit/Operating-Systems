...
A process provides a running programme memory, which contains **instructions, data and a stack.**  
...
Each application in user space is running as a process and have a unique process identifier(PID). A process is prepared for running a programme.
...
`mknod(char *file, int driver, int device)`

Create a device file. Recall the all the devices are file descriptors to the OS, xv6 , too. 

As an illustration, `mknod("console", CONSOLE, 0) // CONSOLE: 1` create a console.
...
(1) The unit of isolation of xv6 and in other Unix systems is a process. This mechanism prevents one process interrupting or doing something harm to other process. The process abstraction make a programme believe that the machine is used by itself only. 

(2) In page table, a virtual address is manipulated by RISC-V instruction and a physical address is that that is sent by CPU chip to main memory. xv6 offers a page table to each process so that it can have its own memory space. As a result, it has an illusion that it has all the memory space in the machine. Hence, the isolation of a process is implemented. 
...
(5) In conclusion, there are two approaches that xv6 adopt to isolate a process: one is page table to give a process an illusion that it has its own memory space; the other is thread which gives a process an illusion that it has its own CPU. 
